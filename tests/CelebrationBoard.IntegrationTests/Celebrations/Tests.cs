...
using CelebrationBoard.Api.Utils;
...
public sealed class Tests : IntegrationTest
{
  ...
  public async Task<T> PostAndDeserialise<T, U>(string uri, U request, HttpStatusCode statusCode, string? token = null)
  {
    if (token is not null)
    {
      _client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", token); // TODO: Does this need to be reset?
    }
  ...
  public async Task Post_celebration_and_delete_it_cannot_be_retrieved()
  {
    ...
    var registerRequest = new RegisterRequest(username, $"{Guid.NewGuid()}@gmail.com", password);
    var registerResponse = await PostAndDeserialise<Envelope<long>, RegisterRequest>(apiV1("register"), registerRequest, HttpStatusCode.OK);

    var loginRequest = new LoginRequest(username, password);
    var loginResponse = await PostAndDeserialise<Envelope<string>, LoginRequest>(apiV1("login"), loginRequest, HttpStatusCode.OK);
    var apiToken = loginResponse.Result;

    var postCelebrationRequest = ValidPostCelebrationRequest(registerResponse.Result);
    var postCelebrationResponse = await PostAndDeserialise<Envelope<PostCelebrationResponse>, PostCelebrationRequest>(apiV1("celebrations"), postCelebrationRequest, HttpStatusCode.Created, apiToken);
    var celebrationId = postCelebrationResponse.Result.Id;

    var deleteCelebrationRequest = new DeleteCelebrationRequest(UserId: registerResponse.Result, CelebrationId: celebrationId);
    var deleteCelebrationResponse = await DeleteAndDeserialise<Envelope, DeleteCelebrationRequest>(apiV1($"celebrations/{celebrationId}"), deleteCelebrationRequest);

    var getAllResponse = await GetAndDeserialise<Envelope<GetCelebrationsResponse>>(apiV1("celebrations"), HttpStatusCode.OK);
    var celebration = getAllResponse.Result.Celebrations.Where(c => c.Id == celebrationId).FirstOrDefault();
